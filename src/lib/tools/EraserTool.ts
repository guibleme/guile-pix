...
export class EraserTool extends BaseTool {
  ...
  private erasePoint(x: number, y: number, ctx: ToolContext): void {
    ...
    for (const point of symmetryPoints) {
      ...
      this.drawBrush(point.x, point.y, ctx, TRANSPARENT);
    }
  }

  private erasePointTracked(
    x: number,
    y: number,
    ctx: ToolContext,
    touched: Map<string, Point>
  ): void {
    this.erasePoint(x, y, ctx);
  ...
  private renderPixelPerfectStroke(ctx: ToolContext): void {
    ...
    for (let i = 0; i < this.pixelPerfectStroke.length; i += 1) {
      ...
      this.erasePointTracked(point.x, point.y, ctx, this.pixelPerfectTouched);
    }
  }
  ...
  private renderAccumulateUpdateLastStroke(ctx: ToolContext): void {
    ...
    if (this.isDotsMode(ctx)) {
      for (const point of this.traceStroke) {
        this.erasePointTracked(point.x, point.y, ctx, this.traceTouched);
      }
      return;
    }

    const first = this.traceStroke[0];
    this.erasePointTracked(first.x, first.y, ctx, this.traceTouched);
